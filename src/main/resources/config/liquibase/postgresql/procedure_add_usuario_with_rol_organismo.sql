CREATE OR REPLACE FUNCTION add_usuario_with_existing_rol(login usuario.login%TYPE, nombre usuario.nombre%TYPE, apellido1 usuario.apellido1%TYPE, 
    ...
    BEGIN
        if upper(namerol) = 'ADMIN' then
            INSERT INTO usuario(id, opt_lock, login, nombre, apellido1, apellido2, email, deletion_date, created_by, created_date, last_modified_by, last_modified_date, is_admin)
            VALUES(nextval('usuario_id_seq'), 0, login, nombre, apellido1, apellido2, email, NULL, 'system', now(), 'system', null, true);
        else
            ...
            select id into id_rol_sel from tb_roles where name = upper(nameRol);
